   ...
   <div class="col-md-8">
      <div class="panel panel-default">
            <div class="panel-heading">
              <h3 class="panel-title">تعديل المشتريات</h3>
            </div>
        ...
        <div class="col-md-12">
          <div class="ls-editable-table table-responsive ls-table">
              <table class="table table-bordered  table-bottomless">
               ...
               <tbody>
                 <?php $total = 0; ?>
               @foreach(json_decode($rows->cash_id) as $key => $cashy)
                   @foreach(App\Cash_other::where('id',$cashy)->get() as $raw)@endforeach
                   @foreach(App\Product::where('pro_code',$raw->pro_code)->get() as $pro) @endforeach
                      <tr>
                          <td class='text-center'>{{$key+1}}</td>
                          <td class='text-center'>{{$raw->pro_code}}</td>
                          <td class='text-center' id="unit_price_{{$pro->id}}">{{$raw->price}}</td>
                          <td class='text-center'>
                            <input type="number" step="any" min="0" onchange="submit_qty_{{$pro->id}}({{$raw->id}});" id="qty_{{$pro->id}}" style="width:80px;padding:3px;" max="{{$pro->qty}}" value="{{$raw->qty}}">
                            <p style='color:red;font-size:11px;'>الكمية المتاحة {{$pro->qty}} فقط</p>
                          </td>
                          <td class="text-center" id="total_price_{{$pro->id}}">{{$raw->price*$raw->qty}}</td>
                          <td class='text-center'>
                              {!! Form::Open(['url'=>'sales/cash/remove-cart/edit/'.$raw->id.'/'.$getid]) !!}
                                 <button class="btn btn-danger" @if($key == 0) disabled @endif><i class="fa fa-trash"></i> حذف</button>
                              {!! Form::Close() !!}
                          </td>    
                      </tr>
                       <?php $total += $raw->price*$raw->qty; ?>
                  @endforeach
            ...
            <div class="col-md-12 row form-group">
               <a href="{{ url('sales/cash/invoice/'.$rows->id) }}" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i> الفاتورة</a>
            </div>  
            <input type="hidden" id="getid" value="{{$rows->id}}">
        ...
        <div class="col-md-12">
            ...
            <div id="search_result" class="form-group"  style="display:none;height:auto">
                <div class="col-md-12">
                  ...
                  {!! Form::Open(['url'=>'sales/cash/add-to-cart/edit/'.$getid]) !!}  
      ...
      {!! Form::Open() !!}
        ...
        <div class="col-md-12">
           <div class="col-md-4 col-md-offset-2 form-group">
              <label>الأسم</label>
              <input type="text" class="form-control" name="name" required value="{{$rows->name}}">
           </div>

           <div class="col-md-4 form-group">
              <label>رقم الهاتف</label>
              <input type="number" class="form-control" name="phone" value="{{$rows->phone}}">
           </div>
        ...
        <div class="col-md-12" style="margin-top:20px;">
           ...
           <div class="col-md-4 form-group">
              ...
              <input type="number" step="any" id="totals" min="0" style="padding:1px;padding-right:10px;" class="form-control" value="{{$rows->total}}" name="total" required>
           </div>

           <div class="col-md-4 form-group">
              <label>الخصم</label>
              <input type="number" step="any" id="discount" min="0" style="padding:1px;padding-right:10px;" onkeyup="calc_remain();" value="{{$rows->discount}}"  class="form-control" name="discount" required>
           </div>

           <div class="col-md-4 form-group">
              <label>الإجمالي بعد الخصم</label>
              <input type="number" id="after_dis" step="any" style="padding:1px;padding-right:10px;" class="form-control" value="{{$rows->total_after_dis}}" name="total_after_dis" required>
           </div>


           <div class="col-md-12 form-group">
              <label>ملاحظات مع الفاتورة</label>
              <textarea name="note" rows="5" class="form-control">{{$rows->notes}}</textarea>
           </div>

        </div>  
<input type="hidden" name="__token" value="{{csrf_token()}}">
        <div class="col-md-12" style="margin-top:60px;">
          <button class="btn btn-primary"><i class="fa fa-check-circle"></i> تعديل الفاتورة</button>
...
$("#calc_total").on('click',function(){
  var id = $("#getid").val();
  var Url = '{{ url("sales/cash/calc-total-cart/edit/") }}/'+id;
...
@foreach(json_decode($rows->cash_id) as $key => $cashy)
   @foreach(App\Cash_other::where('id',$cashy)->get() as $jscode)@endforeach
...
function submit_qty_{{$jsfun->id}}(id){
  ...
  var Url = "{{ url('sales/cash/update-qty/edit/') }}/"+id+'/'+value+'';
