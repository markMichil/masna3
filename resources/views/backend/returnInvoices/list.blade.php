...
<section id="min-wrapper">
  <div id="main-content">
    <div class="container-fluid">
       <div class="row">
          <div class="col-md-12">
          <h3 class="ls-top-header">الفواتير مرتجع</h3>
              <div class="panel panel-default">
                  <div class="panel-heading">
                    <h3 class="panel-title">جميع الفواتير المرتجعة ( {{count($data)}} )</h3>
                  </div>
...
@endif
          ...
          <a href="{{ url('returnInvoices/create') }}" class="btn btn-danger"><i class="fa fa-plus-circle"></i> أضف فاتورة مرتجع جديد</a><br/><br/>
              ...
              <table class="table table-bordered  table-bottomless" id="ls-editable-table">
                <thead>
                    <tr>
                     ...
                     <th class="text-center" style='width:5%;font-weight:bold'>عدد المنتجات</th>
               ...
               <tbody>
                  ...
                  @foreach($data as $key => $row)
                      ...
                      @endif
                          ...
                          <td class='text-center'>{{App\return_invoice_details::where('invoice_id',$row->id)->count()+1}}</td>
                          <td class='text-center'>{{$row->total}}</td>
                          <td class='text-center'>{{$row->paid}}</td>
                          <td class='text-center'>{{$row->remain}}</td>
                          <td class='text-center'>
                            {!! Form::Open(['url'=>'returnInvoices/del/'.$row->id,'id'=>'form']) !!}
                                 <a href="{{ url('returnInvoices/edit/'.$row->id) }}" class="btn btn-success"><i class="fa fa-edit"></i> تعديل</a>
