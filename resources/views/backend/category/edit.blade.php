          ...
          <h3 class="ls-top-header">الأقسام</h3>
              <div class="panel panel-default">
                  <div class="panel-heading">
                    <h3 class="panel-title">تعديل القسم</h3>
                 </div>
        ...
        <div class="col-md-6">
           ...
           <div class="form-group">
              ...
              <select name="parent" class="form-control" style="padding:0;padding-right:5px;">
                      <option value="0" @if($row->parent == 0) selected @endif>قسم رئيسي</option>
                 @foreach(App\Category::where('parent',0)->get() as $cat) 
                      <option value="{{$cat->id}}" @if($row->parent == $cat->id) selected @endif>قسم فرعي من  {{$cat->slug}}</option>
                 @endforeach
              </select>
           </div> 
